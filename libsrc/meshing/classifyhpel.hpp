...
typedef ClosedHashTable<PointIndices<2>, int> HT_EDGES;
typedef ClosedHashTable<SortedPointIndices<2>, int> HT_FACE_EDGES;
typedef ClosedHashTable<SortedPointIndices<3>, int> HT_FACES;


HPREF_ELEMENT_TYPE ClassifyTet(HPRefElement & el, HT_EDGES & edges, HT_EDGEPOINT_DOM & edgepoint_dom, 
                               TBitArray<PointIndex> & cornerpoint, TBitArray<PointIndex> & edgepoint, HT_FACES & faces, HT_FACE_EDGES & face_edges, 
                               HT_FACE_EDGES & surf_edges, Array<int, PointIndex> & facepoint)
{
...
HPREF_ELEMENT_TYPE ClassifyPrism(HPRefElement & el, HT_EDGES & edges, HT_EDGEPOINT_DOM & edgepoint_dom, 
                                 TBitArray<PointIndex> & cornerpoint, TBitArray<PointIndex> & edgepoint, HT_FACES & faces, HT_FACE_EDGES & face_edges, 
                                 HT_FACE_EDGES & surf_edges, Array<int, PointIndex> & facepoint)
{
  ...
  for(int m=1;m<=6;m++)
    {
      ...
      auto eledges = MeshTopology::GetEdges (PRISM);
      for(int k=0;k<9;k++)
        {
          PointIndices<2> i2 = PointIndices<2>(el.PNum(p[eledges[k][0]]),el.PNum(p[eledges[k][1]])).Sort(); 
          if (edges.Used(i2)) edge_sing[k] = 2;
          else edge_sing[k] = face_edges.Used(i2);
        }
      
      auto elfaces = MeshTopology::GetFaces (PRISM);
      for (int k=0;k<5;k++)
        {
          PointIndices<3> i3; 
          
          if(k<2) 
            i3 = PointIndices<3>(el.pnums[p[elfaces[k][0]]-1], el.pnums[p[elfaces[k][1]]-1], 
                                       el.pnums[p[elfaces[k][2]]-1]).Sort(); 
          else 
            { 
              PointIndices<4> i4 (el.pnums[p[elfaces[k][0]]-1], el.pnums[p[elfaces[k][1]]-1],
                                  el.pnums[p[elfaces[k][2]]-1],el.pnums[p[elfaces[k][3]]-1]); 
              i4.Sort();
  ...
  for (int j=0;j<3;j++) 
    {
      ...
      auto eledges = MeshTopology::GetEdges (TRIG); 
      
      if(dim==3)
        {
          for(int k=0;k<3;k++) 
            { 
              int ep1=p[eledges[k][0]];  
              int ep2=p[eledges[k][1]];  
  ...
  for (int j = 1; j <= 4; j++)
    {
      ...
      if(dim ==3) 
        { 
          ...
          i2 = PointIndices<2>(el.PNumMod (j+1), el.PNumMod (j+2));
          // i2.Sort();
          ...
          i2 = PointIndices<2>(el.PNumMod (j+3), el.PNumMod (j+4));
          // i2.Sort();
        ...
        { 
          PointIndices<2> i2;
          i2 = PointIndices<2>(el.PNumMod (j), el.PNumMod (j+1));
          i2.Sort();
          isedge1 = edges.Used (i2);
          if(isedge1)
            {
                               ...
                               HT_FACE_EDGES & surf_edges, Array<int, PointIndex> & facepoint)
{
  ...
  int p[8]; 
  auto elfaces = MeshTopology::GetFaces (HEX);
  auto eledges = MeshTopology::GetEdges (HEX);
  ...
  for(int m=0;m<6 && type == HP_NONE;m++) 
    for(int j=0;j<4 && type == HP_NONE;j++) 
      { 
        ...
        for(int l=0;l<4;l++) 
          {
            p[l] = elfaces[index[m][0]][(4-j-l)%4]+1; 
            p[l+4] = elfaces[index[m][1]][(j+l)%4]+1;
          }
        ...
        for (int k=0;k<6;k++)
          {
            ...
            PointIndices<4>  i4 = PointIndices<4>(el.pnums[p[elfaces[k][0]]-1], el.pnums[p[elfaces[k][1]]-1], el.pnums[p[elfaces[k][2]]-1],el.pnums[p[elfaces[k][3]]-1]); 
            i4.Sort();
            i3 = PointIndices<3>(i4[0], i4[1], i4[2]); 
...
HPREF_ELEMENT_TYPE ClassifyHex7 (HPRefElement & el, HT_EDGES & edges, HT_EDGEPOINT_DOM & edgepoint_dom, 
                                 ...
                                 HT_FACE_EDGES & surf_edges, Array<int, PointIndex> & facepoint)
{
  ...
  PointIndices<4> fbot4 = { el.pnums[0], el.pnums[1], el.pnums[2], el.pnums[3] };
  PointIndices<3> ftop = { el.pnums[4], el.pnums[5], el.pnums[6] };
  fbot4.Sort();
  PointIndices<3> fbot = { fbot4[0], fbot4[1], fbot4[2] };
  ftop.Sort();
...
HPREF_ELEMENT_TYPE ClassifySegm(HPRefElement & hpel, HT_EDGES & edges, HT_EDGEPOINT_DOM & edgepoint_dom, 
                                TBitArray<PointIndex> & cornerpoint, TBitArray<PointIndex> & edgepoint,
                                HT_FACES & faces, HT_FACE_EDGES & face_edges, 
                                HT_FACE_EDGES & surf_edges, Array<int, PointIndex> & facepoint)
{
  ...
  PointIndices<2> i2;
  i2 = PointIndices<2>(hpel[0], hpel[1]);
  i2.Sort();
  if (!edges.Used (i2))
    {
...
HPREF_ELEMENT_TYPE ClassifyPyramid(HPRefElement & el, HT_EDGES & edges, HT_EDGEPOINT_DOM & edgepoint_dom, 
                                   TBitArray<PointIndex> & cornerpoint, TBitArray<PointIndex> & edgepoint,
                                   HT_FACES & faces, HT_FACE_EDGES & face_edges, 
                                   HT_FACE_EDGES & surf_edges, Array<int, PointIndex> & facepoint)
{
  ...
  auto elfaces = MeshTopology::GetFaces (PYRAMID);
  auto eledges = MeshTopology::GetEdges (PYRAMID);
  ...
  for(int m=0;m<4 && type == HP_NONE;m++) 
    {
      *testout << "m = " << m << endl;
      ...
      for(int k=0;k<8;k++)
        {
          PointIndices<2> i2 = PointIndices<2>(el.pnums[p[eledges[k][0]]],
                                       el.pnums[p[eledges[k][1]]]).Sort(); 
          if (edges.Used(i2)) 
            edge_sing[k] = 2;
          else 
      ...
      for (int k=0;k<5;k++)
        {
          PointIndices<3> i3;
          /*
          PointIndices<4> i4 = PointIndices<4>(el.pnums[p[elfaces[k][0]]], el.pnums[p[elfaces[k][1]]], el.pnums[p[elfaces[k][2]]],
                                el.pnums[p[elfaces[k][3]]]); 
          i4.Sort();
          i3 = PointIndices<3>(i4.I1(), i4.I2(), i4.I3()); 
          */
          if (k < 4)
            {
              i3 = PointIndices<3>(el.pnums[p[elfaces[k][0]]], el.pnums[p[elfaces[k][1]]], el.pnums[p[elfaces[k][2]]]);
              i3.Sort();
            }
          else
            {
