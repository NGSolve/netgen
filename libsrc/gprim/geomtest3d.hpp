 ...
 */
extern int 
IntersectTetTriangleRef (const Point<3> ** tri, const int * tripi = NULL);
...
extern int
CalcTriangleCenter (const Point<3> ** pts, Point<3> & c);
...
*/
extern double ComputeCylinderRadius (const Point<3> & p1, const Point<3> & p2,
...
*/
extern double ComputeCylinderRadius (const Vec<3> & n1, const Vec<3> & n2,
				     double h1, double h2);

/// Minimal distance of point p to the line segment [lp1,lp2]
DLL_HEADER double MinDistLP2 (const Point<2> & lp1, const Point<2> & lp2, const Point<2> & p);

/// Minimal distance of point p to the line segment [lp1,lp2]
DLL_HEADER double MinDistLP2 (const Point<3> & lp1, const Point<3> & lp2, const Point<3> & p);

/// Minimal distance of point p to the triangle segment [tp1,tp2,pt3]
DLL_HEADER double MinDistTP2 (const Point<3> & tp1, const Point<3> & tp2, 
...
extern double MinDistLL2 (const Point<3> & l1p1, const Point<3> & l1p2,
			  const Point<3> & l2p1, const Point<3> & l2p2);
