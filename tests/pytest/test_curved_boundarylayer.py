...
class TestThickBL:
    """Phase 4+5: thick BL (h/R large) must still have valid elements."""

    @pytest.mark.parametrize("order", [2, 3])
    def test_thick_bl_positive_jacobian(self, order):
        """Sphere with thick BL (h/R = 0.3): all elements must have positive
        Jacobian after curving."""
        ngs = pytest.importorskip("ngsolve")
        r = 1.0
        h = 0.3  # 30% of radius — very thick
        geo, bl, maxh = make_sphere_with_bl(r=r, bl_thickness=[h], maxh=0.3)
        ngmesh = geo.GenerateMesh(maxh=maxh, boundary_layers=bl)
        mesh = ngs.Mesh(ngmesh)
        mesh.Curve(order)

        # Integrate indicator: 1 where det <= 0, 0 elsewhere.
        # Result should be 0 (no volume with negative Jacobian).
        jac_det = ngs.Det(ngs.specialcf.JacobianMatrix(3))
        bad_vol = ngs.Integrate(ngs.IfPos(jac_det, 0, 1), mesh)
        assert bad_vol == pytest.approx(0, abs=1e-10), \
            f"Found elements with non-positive Jacobian (bad volume = {bad_vol})"

    def test_thick_bl_volume(self):
        """Thick BL volume must still be reasonably accurate."""
        ngs = pytest.importorskip("ngsolve")
        r = 1.0
        h = 0.3
        geo, bl, maxh = make_sphere_with_bl(r=r, bl_thickness=[h], maxh=0.3)
        ...
        mesh = ngs.Mesh(ngmesh)
        mesh.Curve(3)
        vol = ngs.Integrate(1.0, mesh)
        expected = sphere_volume(r)
        assert vol == pytest.approx(expected, rel=0.02), \
            f"Thick BL sphere volume {vol} != expected {expected}"


# ---------------------------------------------------------------------------
# T6: Press-through prevention
# ---------------------------------------------------------------------------

class TestPressThrough:
    """Phase 5: curving must not create inverted elements, even in extreme cases."""

    def test_small_sphere_thick_bl(self):
        """Small sphere with relatively thick BL and high order curving.
        The limiter must kick in to prevent press-through."""
        ngs = pytest.importorskip("ngsolve")
        r = 0.5
        h = 0.15  # 30% of radius
        geo, bl, maxh = make_sphere_with_bl(r=r, bl_thickness=[h], maxh=0.25)
        ngmesh = geo.GenerateMesh(maxh=maxh, boundary_layers=bl)
        mesh = ngs.Mesh(ngmesh)
        mesh.Curve(4)
        ...
        bad_vol = ngs.Integrate(ngs.IfPos(jac_det, 0, 1), mesh)
        assert bad_vol == pytest.approx(0, abs=1e-10), \
            "Press-through detected: elements with non-positive Jacobian"

    def test_coarse_sphere_bl(self):
        """Coarse mesh on sphere with BL — worst case for press-through."""
        ngs = pytest.importorskip("ngsolve")
        r = 1.0
        h = 0.2
        geo, bl, maxh = make_sphere_with_bl(r=r, bl_thickness=[h], maxh=0.8)
        ngmesh = geo.GenerateMesh(maxh=maxh, boundary_layers=bl)
        mesh = ngs.Mesh(ngmesh)
        mesh.Curve(3)
        ...
        bad_vol = ngs.Integrate(ngs.IfPos(jac_det, 0, 1), mesh)
        assert bad_vol == pytest.approx(0, abs=1e-10), \
            "Press-through detected on coarse sphere BL mesh"
